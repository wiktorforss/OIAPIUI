...
function SummaryCard({ label, value, sub, color }: { label: string; value: string; sub?: string; color?: string }) {
  return (
    <div className="card">
      ...
      <p className={clsx("text-xl font-bold mt-1", color ?? "text-gray-100")}>{value}</p>
...
export default function PortfolioPage() {
  ...
  const totalPnlPos     = summary.total_pnl >= 0;
              ...
              {openPositions.map(p => (
                <div key={p.ticker} className="bg-gray-800/50 rounded-xl p-3 space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <PnlBadge value={p.unrealized_pnl} pct={p.unrealized_pct} />
                  </div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {closedPositions.map(p => (
                      <tr key={p.ticker} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                        <td className="td"><TickerLink ticker={p.ticker} /></td>
                        <td className="td"><PnlBadge value={p.realized_pnl} /></td>
